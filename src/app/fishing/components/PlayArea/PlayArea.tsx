...
function PlayArea(props: PlayAreaProps) {
  ...
  const updateFishMovement = useCallback((currentVector: Vector) => {
    const boxWidth =
      document.getElementById("play-area")?.clientWidth ?? window.innerWidth;
    const boxHeight =
      document.getElementById("play-area")?.clientHeight ?? window.innerHeight;

    const imageElement = document.querySelector("img");
    const fishWidth =
      imageElement?.getBoundingClientRect().width ?? DEFAULT_ICON_SIZE;
    const fishHeight =
      imageElement?.getBoundingClientRect().height ?? DEFAULT_ICON_SIZE;
    ...
    const hasXBorderCollision =
      nextVector.x <= 0 || nextVector.x >= boxWidth - fishWidth;
    const hasYBorderCollision =
      nextVector.y <= 0 || nextVector.y >= boxHeight - fishHeight;

    if (hasXBorderCollision) {
      nextVector.dx =
        nextVector.x > 0 ? getRandomInt(-5, 0) : getRandomInt(0, 5);
    }

    if (hasYBorderCollision) {
      nextVector.dy =
        nextVector.y > 0 ? getRandomInt(-5, 0) : getRandomInt(0, 5); // Reverse vertical direction
    }

    return nextVector;
  }, []);

  const areFishAndHookColliding = (
    fishRect: DOMRect,
    hookRect: DOMRect
  ): boolean => {
  ...
  return (
    ...
    >
      {props.isReelCast && (
        <HookGameplayElement hookPosition={props.hookPosition} />
      )}
