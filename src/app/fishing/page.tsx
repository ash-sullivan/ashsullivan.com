...
export default function FishingGame() {
    ...
    return (
      <div className="modal">
        <p className="text-center mx-auto m-2 p-2">
          ...
          <br />
          The game isn&apos;t very exciting right now, but stay tuned for more
          fun!
        </p>
        <center>
          <button className="button mt-4" onClick={onClose}>
            OK!
          </button>
        </center>
      </div>
  ...
  return (
    <div
      id="main"
      className="min-h-screen min-w-screen p-4 gap-4 items-center font-[family-name:var(--font-geist-sans)]"
    >
      <header>
      ...
      <main className="grid justify-center">
        {isRulesModalOpen &&
          createPortal(
            <RulesContent onClose={() => setIsRulesModalOpen(false)} />,
            document.body
          )}
        <p className="text-center">Fish Caught: {score}</p>
        {isFishCaught ? (
          <p className="text-center">You caught a fish!</p>
        ) : (
          <br />
        )}
            ...
            onClick={
              isFishCaught
                ...
                : (coordinates: { x: number; y: number }) =>
                    isReelCast ? reelIn() : castReel(coordinates)
            }
        ...
        <button className="button" onClick={() => setIsRulesModalOpen(true)}>
          Rules
        </button>
      </main>
    </div>
