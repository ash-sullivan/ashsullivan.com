...
export default function SlotMachine() {
  ...
  useEffect(() => {
    if (isSlotLocked.every((locked) => locked)) {
      if (
        slotSelections.every((slot) => slot > 0 && slot === slotSelections[0])
      ) {
        setHasWon(true);
      ...
      <div className="justify-center flex flex-row flex-wrap gap-4 px-80 pt-8 text-4xl">
        {slotSelections.map((slotSelection, index) => {
          return (
            <div
              key={index}
              className="relative border border-white rounded p-4"
            >
              {slotSelection}
            </div>
          );
        ...
        {hasWon && (
          <p className="text-center text-2xl pt-8">
            You won! You now have {numberOfWins}{" "}
            {numberOfWins === 1 ? "win" : "wins"}.
          </p>
        )}
      </div>
    </main>
